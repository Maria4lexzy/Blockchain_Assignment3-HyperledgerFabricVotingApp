...
const electionDataPath = path.join(
    process.cwd(),
    "./lib/data/electionData.json"
);
...
class MyAssetContractContract extends Contract {
    ...
    async init(ctx) {
        ...
        for (let i = 0; i < votableItems.length; i++) {
            ...
            await ctx.stub.putState(
                votableItems[i].votableId,
                Buffer.from(JSON.stringify(votableItems[i]))
            );
        }
    ...
    async createVoter(ctx, args) {
        ...
        let votableItems = JSON.parse(
            await this.queryByObjectType(ctx, "votableItem")
        );
    ...
    async castVote(ctx, args) {
        ...
        if (electionExists) {
            ...
            if (
                ...
                let result = await ctx.stub.putState(
                    votableId,
                    Buffer.from(JSON.stringify(votable))
                );
                console.log(result);
                ...
                let response = await ctx.stub.putState(
                    voter.voterId,
                    Buffer.from(JSON.stringify(voter))
                );
